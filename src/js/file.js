...
export function fileCompress(file, maxFileSize, startingQuality = 0.9) {
  return new Promise((resolve) => {
    if (file.type !== 'image/png' && file.type !== 'image/jpeg' && file.type !== 'image/webp') {
      resolve({ file, quality: startingQuality });
      return;
    }
...
function changeRandomPixel(file, quality = 0.9) {
  return new Promise((resolve) => {
...
function addRandomDataToFile(file) {
  return new Promise((resolve) => {
    debugLog('Adding random bytes to the file');
...
export function anonFile(file, quality = 0.9) {
  if (file.type === 'image/jpeg' || file.type === 'image/png') {
    return changeRandomPixel(file, quality);
  }
  return addRandomDataToFile(file);
}

function drawWatermarkReddit(file, quality = 0.9) {
  return new Promise((resolve) => {
    const reader = new FileReader();
    reader.addEventListener('load', () => {
      const img = new Image();
      img.onload = () => {
        ...
        let watermarkH = img.height * 0.08;

        if (watermarkH < 30) watermarkH = 30;
        ...
        // reddit uses system's fonts, on android it's Roboto
        const fontName = 'SF Compact Display';
        const fontWeightLight = 300;
        const fontWeightMedium = 500;

        const urlFontLight = browser.runtime.getURL('/fonts/SF-Compact-Display-Light.otf');
        const urlFontMedium = browser.runtime.getURL('/fonts/SF-Compact-Display-Medium.otf');

        const fontLight = new FontFace(fontName, `url(${urlFontLight})`, { weight: fontWeightLight });
        const fontMedium = new FontFace(fontName, `url(${urlFontMedium})`, { weight: fontWeightMedium });
        ...
        document.fonts.ready.then(() => {
          ...
          const watermarkHalf = watermarkH * 0.5;
          const watermarkMid = img.height + watermarkHalf;

          // reddit uses ~3:1 ratio but firefox 122 can't stretch vectors using drawImage
          // reddit.png's width and height
          const logoAspectRatio = 200 / 56;

          // if we don't round, firefox crops a pixel at the bottom and on the side
          const logoH = Math.round(watermarkH * 0.39); // pre redesign multiplier was 0.47
          const logoW = Math.round(logoH * logoAspectRatio);

          const logoX = img.width - sidePadding - logoW;
          const logoY = watermarkMid - logoH * 0.5;

          let fontSize = Math.round(watermarkH * 0.5);
          let font = `${fontSize}px '${fontName}'`;

          ctx.font = `${fontWeightLight} ${font}`;
          ctx.fillStyle = 'white';
          ...
          const subreddits = [
            ...
            '4chan', 'pepethefrog', 'Apustaja', 'me_irl', '196', 'ihaveihaveihavereddit',
          ];
          ...
          const subreddit = subreddits[getRandomInt(0, subreddits.length)];
          const username = usernames[getRandomInt(0, usernames.length)];

          // make sure text doesn't overlap logo
          const textW = ctx.measureText(`Posted in r/${subreddit} by u/${username}`).width;
          const totalW = sidePadding + textW + img.width * 0.15;
          if (logoX < totalW) {
          ...
          const textMetrics = ctx.measureText('Posted in r');
          const textH = textMetrics.actualBoundingBoxAscent
            + textMetrics.actualBoundingBoxDescent;
          // default textBaseline is bottom ("alphabetic")
          const textY = img.height + watermarkHalf + textH * 0.5;

          let textX = sidePadding;

          const postedIn = 'Posted in r/';
          ctx.fillText(postedIn, textX, textY);
          textX += ctx.measureText(postedIn).width;

          ctx.font = `${fontWeightMedium} ${font}`;
          ctx.fillText(subreddit, textX, textY);
          textX += ctx.measureText(subreddit).width;

          ctx.font = `${fontWeightLight} ${font}`;
          const by = ' by u/';
          ctx.fillText(by, textX, textY);
          ...
          logo.onload = () => {
            // after the redesign they fucked up their logo
            // and now they seem to be using a png rather than an svg
...
function drawWatermarkIfunny(file, quality = 0.9) {
  return new Promise((resolve) => {
    const reader = new FileReader();
    reader.addEventListener('load', () => {
      const img = new Image();
...
function bypassNSFWfilter(file, quality = 0.9) {
  return new Promise((resolve) => {
    debugLog('Bypassing NSFW filter');
    ...
    reader.addEventListener('load', () => {
      ...
      img.onload = () => {
        ...
        const borderWidth = Math.round((img.width * 0.03) / color.length) * color.length;

        const cvs = document.createElement('canvas');
        cvs.width = img.width + borderWidth * 2;
        cvs.height = img.height + borderWidth * 2;

        const ctx = cvs.getContext('2d');
        ctx.lineWidth = borderWidth / color.length;

        for (let i = 0; i < color.length; ++i) {
          ctx.strokeStyle = color[i];
          ctx.strokeRect(
            ctx.lineWidth / 2 + (ctx.lineWidth * i),
            ctx.lineWidth / 2 + (ctx.lineWidth * i),
            cvs.width - ctx.lineWidth * (i * 2 + 1),
            cvs.height - ctx.lineWidth * (i * 2 + 1),
          );
        }
...
export function drawWatermark(file, quality, type) {
  if (file.type !== 'image/jpeg' && file.type !== 'image/png') {
    return Promise.resolve(file);
  }

  if (type === 'reddit') return drawWatermarkReddit(file, quality);
  if (type === 'ifunny') return drawWatermarkIfunny(file, quality);
  if (type === 'nsfwfilterbypass') return bypassNSFWfilter(file, quality);

  return Promise.resolve(file);
}

function getRandomDatePastYear() {
  const timestamp = new Date().getTime();
  return new Date(timestamp - getRandomInt(0, 365 * 24 * 60 * 60 * 1000));
}
...
export function fakeFilename(file, type) {
  ...
  switch (type) {
    // 1704194250189220
    case 'unix': {
      newFilename = `${getRandomDatePastYear().getTime()}${
        getRandomString(3, false, false, true)}`;
      break;
    }
    // 0e9
    case 'kym': {
    ...
    case 'twitter': {
      const prefixes = ['F', 'GC'];
      const randomPrefix = prefixes[getRandomInt(0, prefixes.length)];
    ...
    case 'whatsapp': {
      const date = getRandomDatePastYear();
      let month = date.getMonth() + 1;
      if (month < 10) month = `0${month}`;
      let day = date.getDate();
      if (day < 10) day = `0${day}`;
      newFilename = `IMG-${date.getFullYear()}${month}${
        day}-WA00${getRandomString(2, false, false, true)}`;
      break;
    }
    default: {
      return file;
  ...
  if (lastDotIndex !== -1) {
    newFilename += file.name.slice(lastDotIndex);
  }
