  ...
  return new Promise((resolve) => {
    ...
    reader.addEventListener('load', () => {
      ...
      img.onload = () => {
        ...
        const channelIndex = Math.floor(Math.random() * (data.length / 4)) * 4;
        data[channelIndex] += ((data[channelIndex] === 255) ? -1 : 1);
...
function getRandomDatePastYear() {
  ...
  return new Date(timestamp - Math.floor(Math.random() * 365 * 24 * 60 * 60 * 1000));
}
